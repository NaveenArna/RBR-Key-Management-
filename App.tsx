...
interface TxLog { id: string; ts: string; type: string; address: string; box: string; keyType: string; qty: number; }
interface AddrEntry { addr: string; box: string; }

// ─────────────────────────────────────────────────────────────────────────────
// Helpers
// ─────────────────────────────────────────────────────────────────────────────
function totalKeys(r: PropertyRow): number {
  return KEY_TYPES.reduce((s,k) => s + (r[k] || 0), 0);
}
function nowStr(): string {
  return new Date().toLocaleString("en-IN",{dateStyle:"short",timeStyle:"medium"});
}
function pctColor(p: number): string {
  return p >= 100 ? "#e06060" : p >= 70 ? "#c8960c" : "#50c880";
}
...
function similarity(a: string, b: string): number {
  ...
  const lev=ml>0?1-levenshtein(al,bl)/ml:1;
  return tok*0.7+lev*0.3;
}
...
function getBoxStats(data: Record<string,PropertyRow[]>, settings: Record<string,{maxProps:number}>): Record<string,BoxStat> {
  const r: Record<string,BoxStat> = {};
  for(const [box,rows] of Object.entries(data)){
    const keys=rows.reduce((s,row)=>s+totalKeys(row),0);
...
function seedData(raw: Record<string,any[]>): Record<string,PropertyRow[]> {
  const d: Record<string,PropertyRow[]> = {};
  for(const [box,rows] of Object.entries(raw))
    d[box]=rows.map((r,i)=>({...r,_id:r._id||`${box}-${i}`}));
  return d;
}
...
interface AddressInputProps {
  value: string;
  onChange: (v:string, box?:string) => void;
  onSelect: (addr:string, box:string) => void;
  onAddNew: (query:string, box:string) => void;
  allEntries: AddrEntry[];
  boxes: string[];
  boxStats: Record<string,BoxStat>;
  activeBox?: string;
  placeholder?: string;
}
function AddressInput({value,onChange,onSelect,onAddNew,allEntries,boxes,boxStats,activeBox,placeholder}: AddressInputProps) {
  ...
  const grouped=useMemo(()=>{
    const g: Record<string,(typeof sugs)>={}; 
    for(const s of sugs){if(!g[s.box])g[s.box]=[];g[s.box].push(s);}
    return g;
  },[sugs]);
  ...
  function pick(addr:string,box:string){
    setQuery(addr);setConfirmed(true);setOpen(false);setPickingBox(false);
    onSelect(addr,box);onChange(addr,box);
  }
  function confirmBox(box:string){
    if((boxStats[box]?.pct||0)>=100) return;
    setOpen(false);setPickingBox(false);onAddNew(query,box);
  }
  ...
  const icon=confirmed?<span style={{color:"#50c050"}}>✓</span>
    :isNew?<span style={{color:"#c8960c"}}>✦</span>
    :query.length>=2?<span style={{color:"#806040"}}>⌕</span>:null;
    ...
    <div ref={ref} style={{position:"relative"}}>
      <div style={{position:"relative"}}>
        <input value={query} onChange={handleChange}
          ...
          placeholder={placeholder||"Type to search all boxes..."}
          autoComplete="off" autoCorrect="off" spellCheck={false}
          style={{width:"100%",paddingRight:36,
            borderColor:confirmed?"#3a8a3a":isNew?"#a07020":undefined,
            transition:"border-color .2s"}}/>
        <div style={{position:"absolute",right:10,top:"50%",transform:"translateY(-50%)",fontSize:15,pointerEvents:"none"}}>{icon}</div>
      </div>

      {showDD&&(
        <div style={{position:"absolute",top:"calc(100% + 5px)",left:0,right:0,zIndex:9999,
          background:"#141410",border:"1px solid #3a3020",borderRadius:8,
          boxShadow:"0 10px 36px rgba(0,0,0,.78)",overflow:"hidden",animation:"ddFade .15s ease",
          ...
          {pickingBox?(
            <>
              <div style={{padding:"10px 14px 8px",background:"#0f0f0a",borderBottom:"1px solid #252510",display:"flex",alignItems:"center",gap:8}}>
                ...
                <div>
                  <div style={{fontSize:12,color:"#c8960c",fontWeight:"bold"}}>Which box for this property?</div>
                  <div style={{fontSize:10,color:"#605040",marginTop:1}}>"{query}"</div>
                </div>
              </div>
              <div style={{padding:8}}>
                {boxes.map(box=>{
                  ...
                  return (
                    <div key={box} onClick={()=>!full&&confirmBox(box)} style={{
                      display:"flex",alignItems:"center",justifyContent:"space-between",
                      padding:"12px 14px",borderRadius:6,cursor:full?"not-allowed":"pointer",
                      marginBottom:4,border:`1px solid ${full?"#5a2020":"#2a2010"}`,
                      opacity:full?0.65:1,transition:"all .12s"}}
                      ...
                      <div style={{display:"flex",alignItems:"center",gap:10}}>
                        ...
                        <div>
                          ...
                          <div style={{fontSize:10,color:full?"#c05050":"#604030",marginTop:2}}>
                            {full?`⚠ FULL — ${st.props}/${st.maxProps} properties`:`${st.props}/${st.maxProps} props · ${st.pct}% used`}
                          </div>
                          <div style={{width:110,height:3,background:"#2a2010",borderRadius:2,marginTop:4}}>
                            <div style={{width:`${st.pct}%`,height:"100%",background:pc,borderRadius:2}}/>
                          </div>
                        </div>
                      </div>
                      <div style={{fontSize:11,color:full?"#c05050":"#c8960c",
                        border:`1px solid ${full?"rgba(200,80,80,.3)":"rgba(200,150,12,.3)"}`,
                        padding:"4px 12px",borderRadius:4,flexShrink:0,marginLeft:12}}>
                        {full?"Full 🔒":"Select →"}
                      </div>
                    </div>
                  );
            ...
            <>
              <div style={{padding:"7px 14px 6px",fontSize:9,color:"#806040",letterSpacing:1.5,
                borderBottom:"1px solid #252510",display:"flex",justifyContent:"space-between",background:"#0f0f0a"}}>
                <span>🔍 SEARCHING ALL BOXES</span><span style={{color:"#504030"}}>{sugs.length} found</span>
              </div>
              {Object.entries(grouped).map(([box,items])=>(
                <div key={box}>
                  <div style={{padding:"5px 14px 3px",fontSize:9,color:"#c8960c",letterSpacing:1,
                    background:"rgba(200,150,12,.05)",borderBottom:"1px solid #1e1e10"}}>
                    📦 {box}{activeBox&&box!==activeBox?<span style={{color:"#806040"}}> (other box)</span>:""}
                  </div>
                  {items.map((s,i)=>(
                    <div key={s.addr} onClick={()=>pick(s.addr,s.box)} style={{
                      padding:"10px 14px",cursor:"pointer",borderBottom:"1px solid #1a1a10",
                      display:"flex",alignItems:"center",justifyContent:"space-between"}}
                      ...
                      <div style={{fontSize:11,color:"#c8960c",border:"1px solid rgba(200,150,12,.3)",
                        padding:"3px 10px",borderRadius:4,whiteSpace:"nowrap",marginLeft:12,flexShrink:0}}>Use →</div>
                    </div>
                  ))}
                </div>
              ))}
              {isNew&&(
                <div onClick={()=>setPickingBox(true)} style={{
                  padding:"11px 14px",cursor:"pointer",
                  borderTop:sugs.length>0?"2px solid #2a2010":"none",
                  display:"flex",alignItems:"center",justifyContent:"space-between",
                  background:"rgba(200,150,12,.03)"}}
                  ...
                  <div style={{fontSize:11,color:"#50c880",border:"1px solid rgba(80,200,128,.35)",
                    padding:"3px 10px",borderRadius:4,whiteSpace:"nowrap",marginLeft:12,flexShrink:0}}>Add New →</div>
                </div>
              )}
            </>
          )}
...
function LockScreen({ onUnlock, mode = "unlock", onCancel }: LockScreenProps) {
  ...
  function triggerShake() {
    setShake(true); setTimeout(() => { setShake(false); setDigits([]); }, 500);
  }
  ...
  function handleDigit(d: string) {
    ...
    setDigits(next);
    if (next.length === 4) {
      setTimeout(() => evaluate(next.join("")), 120);
    }
  }

  function evaluate(pin: string) {
    if (step === "enter") {
      if (pin === getStoredPasscode()) {
        setSuccess(true);
  ...
  const dots = [0,1,2,3].map(i => (
    <div key={i} style={{
      ...
      background: i < digits.length
        ? (success ? "#50c880" : shake ? "#e06060" : "#c8960c")
        : "transparent",
  ...
  return (
    <div style={{
      position: "fixed", inset: 0, background: "#0a0a08", zIndex: 9999,
      display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center",
      fontFamily: '"Courier New", monospace',
      animation: "fadeInLock .3s ease"
    }}>
      <style>{`
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 72px)", gap: 14 }}>
        {keys.flat().map((k, i) => (
          k === "" ? <div key={i}/> :
          k === "⌫" ? (
            <div key={i} className="pin-key" onClick={handleDelete} style={{ fontSize: 20, color: "#806040" }}>⌫</div>
          ) : (
...
export default function App() {
  ...
  const [data,setData]=useState<Record<string,PropertyRow[]>>(()=>seedData(INITIAL_DATA));
  const [boxSettings,setBoxSettings]=useState<Record<string,{maxProps:number}>>(()=>{
    const s: Record<string,{maxProps:number}> = {};
    for(const box of Object.keys(INITIAL_DATA)) s[box]={maxProps:DEFAULT_MAX};
    return s;
  });
  ...
  const [tabHistory,setTabHistory]=useState<string[]>([]); // for back navigation
  const [activeBox,setActiveBox]=useState(Object.keys(INITIAL_DATA)[0]);
  ...
  const [txBox,setTxBox]=useState(Object.keys(INITIAL_DATA)[0]);
  const [txKeyType,setTxKeyType]=useState<string>(KEY_TYPES[0]);
  const [txQty,setTxQty]=useState<string>("1"); // store as string so backspace works
  const [txMode,setTxMode]=useState<"deposit"|"withdraw">("withdraw");

  // Add form
  const [addAddress,setAddAddress]=useState("");
  const [addBox,setAddBox]=useState(Object.keys(INITIAL_DATA)[0]);
  ...
  useEffect(()=>{
    ...
    function resetTimer(){
      ...
      inactivityTimer.current=setTimeout(()=>setLocked(true), AUTO_LOCK_MS);
    }
  ...
  function navigateTo(tab:string){
    setTabHistory(prev=>[...prev, activeTab]); // push current before leaving
    setActiveTab(tab);
  }
  function goBack(){
    setTabHistory(prev=>{
      const hist=[...prev];
      const last=hist.pop()||"inventory";
      setActiveTab(last);
      return hist;
    });
  }
  ...
  function addLog(type:string,address:string,box:string,keyType:string,qty:number){
    setTxLog(prev=>[{id:`log-${Date.now()}`,ts:nowStr(),type,address,box,keyType,qty},...prev]);
  }
  ...
  function handleTransaction(){
    ...
    if(txMode==="deposit"&&(boxStats[txBox]?.pct||0)>=100){
      showToast(`⚠ ${txBox} is FULL — choose another box`,"error");return;
    }
    const updated=[...data[txBox]];
    updated[idx]={...row,[txKeyType]:txMode==="withdraw"?current-qty:current+qty};
    setData(prev=>({...prev,[txBox]:updated}));
  ...
  function handleAddNew(){
    ...
    if(existIdx!==-1){
      // Sum into existing
      if(qty===0){showToast("Enter at least 1 key to add","error");return;}
      const row=data[addBox][existIdx];
      const updated={...row};
      KEY_TYPES.forEach(k=>{updated[k]=(row[k]||0)+(parseInt(String(addKeys[k]))||0);});
      const rows=[...data[addBox]];
      rows[existIdx]=updated;
      setData(prev=>({...prev,[addBox]:rows}));
      addLog("deposit",addr,addBox,"(multiple)",qty);
      showToast(`✓ Added ${qty} key(s) to existing "${addr}"`);
    } else {
      // New property
      if((boxStats[addBox]?.pct||0)>=100){showToast(`⚠ ${addBox} is FULL — choose another box`,"error");return;}
      const nextNo=Math.max(...(data[addBox]||[]).map(r=>r["Lock Box No."]),0)+1;
      const newRow: PropertyRow={
        "Lock Box No.":addLockNo?parseInt(addLockNo):nextNo,
        "Property Address":addr,_id:`${addBox}-new-${Date.now()}`,
        ...Object.fromEntries(KEY_TYPES.map(k=>[k,parseInt(String(addKeys[k]))||0])) as any,
      };
  ...
  function handleAddBox(){
    ...
    setBoxSettings(prev=>({...prev,[name]:{maxProps:parseInt(String(newBoxMax))||DEFAULT_MAX}}));
    setActiveBox(name);setShowNewBox(false);setNewBoxName("");setNewBoxMax("100");
    showToast(`✓ Created ${name}`);
  }

  function handleSaveBoxSettings(){
  ...
  function exportXLSX(){
    ...
    if(txLog.length>0){
      ...
      ws2["!cols"]=[{wch:18},{wch:10},{wch:42},{wch:10},{wch:16},{wch:6}];
  ...
  const CSS=`
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .mo{position:fixed;inset:0;background:rgba(0,0,0,.82);display:flex;align-items:center;justify-content:center;z-index:1000}
  ...
  return (
    <div style={{fontFamily:'"Courier New",monospace',background:"#0f0f0f",minHeight:"100vh",color:"#e8e0d0"}}>
      ...
      {/* ── Header ── */}
      <div style={{background:"#0a0a08",borderBottom:"2px solid #2a2010",padding:"13px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:10}}>
        <div style={{display:"flex",alignItems:"center",gap:8,flexShrink:0}}>
          {/* Back button — visible when not on inventory */}
          {activeTab!=="inventory"&&(
            <button onClick={goBack} style={{
              background:"transparent",border:"1px solid #3a3020",color:"#c8960c",
              borderRadius:6,padding:"6px 10px",cursor:"pointer",fontSize:14,
              fontFamily:'"Courier New",monospace',fontWeight:"bold",
              display:"flex",alignItems:"center",gap:4,flexShrink:0,
              transition:"all .15s"
            }}
            onMouseEnter={e=>{(e.currentTarget as HTMLElement).style.background="rgba(200,150,12,.1)";}}
            onMouseLeave={e=>{(e.currentTarget as HTMLElement).style.background="transparent";}}
            title={`Back to ${tabHistory[tabHistory.length-1]||"inventory"}`}>
              ← Back
            </button>
          )}
          {/* Home button — always visible */}
          <button onClick={()=>{setTabHistory([]);setActiveTab("inventory");}} style={{
            background:activeTab==="inventory"?"rgba(200,150,12,.15)":"transparent",
            border:"1px solid #3a3020",color:activeTab==="inventory"?"#c8960c":"#806040",
            borderRadius:6,padding:"6px 10px",cursor:"pointer",fontSize:14,
            fontFamily:'"Courier New",monospace',fontWeight:"bold",
            display:"flex",alignItems:"center",gap:4,flexShrink:0,
            transition:"all .15s"
          ...
          onMouseEnter={e=>{(e.currentTarget as HTMLElement).style.background="rgba(200,150,12,.1)";}}
          onMouseLeave={e=>{(e.currentTarget as HTMLElement).style.background=activeTab==="inventory"?"rgba(200,150,12,.15)":"transparent";}}
      ...
      {/* ── Stats bar ── */}
      <div style={{background:"#0c0c0a",borderBottom:"1px solid #1e1e10",padding:"12px 24px",display:"flex",gap:12,flexWrap:"wrap"}}>
        <div className="sc" style={{flex:1,minWidth:110}}>
          <div style={{fontSize:21,fontWeight:"bold",color:"#c8960c"}}>{Object.values(data).flat().length}</div>
          <div style={{fontSize:10,color:"#806040",letterSpacing:1,marginTop:2}}>PROPERTIES</div>
        </div>
        <div className="sc" style={{flex:1,minWidth:110}}>
          <div style={{fontSize:21,fontWeight:"bold",color:"#90c870"}}>{Object.values(data).flat().reduce((s,r)=>s+totalKeys(r),0)}</div>
          <div style={{fontSize:10,color:"#806040",letterSpacing:1,marginTop:2}}>TOTAL KEYS</div>
        </div>
        {Object.entries(boxStats).map(([box,s])=>{
          const pc=pctColor(s.pct);
          return (
            <div key={box} className="sc" style={{flex:1,minWidth:130,cursor:"pointer"}} onClick={()=>{setActiveBox(box);navigateTo("inventory");}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                <div style={{fontSize:21,fontWeight:"bold",color:s.pct>=100?"#e06060":"#7090d0"}}>{s.props}</div>
                <div style={{fontSize:10,color:pc,background:"rgba(200,150,12,.08)",padding:"2px 6px",borderRadius:4}}>{s.pct}%</div>
              </div>
              <div style={{fontSize:10,color:"#806040",letterSpacing:1,marginTop:2}}>{box}{s.pct>=100?" 🔒":""}</div>
              <div style={{fontSize:10,color:"#504030"}}>{s.props}/{s.maxProps} properties</div>
              <div className="pb"><div className="pf" style={{width:`${s.pct}%`,background:pc}}/></div>
            </div>
          );
        })}
        <div className="sc" style={{flex:1,minWidth:110}}>
          <div style={{fontSize:21,fontWeight:"bold",color:"#7080c0"}}>{txLog.length}</div>
          <div style={{fontSize:10,color:"#806040",letterSpacing:1,marginTop:2}}>TRANSACTIONS</div>
        </div>
      </div>

      <div style={{display:"flex",minHeight:"calc(100vh - 164px)"}}>
        {/* ── Sidebar ── */}
        <div style={{width:215,background:"#0c0c0a",borderRight:"1px solid #1e1e10",padding:"16px 0",flexShrink:0}}>
          <div style={{padding:"0 14px 8px",fontSize:10,color:"#504030",letterSpacing:2}}>NAVIGATE</div>
          {(["inventory","enter","withdraw","add","history"] as const).map(id=>{
            const labels: Record<string,string> = {inventory:"📋 Inventory",enter:"⬇ Deposit Keys",withdraw:"⬆ Withdraw Keys",add:"➕ Add Property",history:"📜 History"};
            return (
              <div key={id} className="ni" onClick={()=>{navigateTo(id);if(id==="enter")setTxMode("deposit");if(id==="withdraw")setTxMode("withdraw");}}
                ...
                {labels[id]}
                {id==="history"&&txLog.length>0&&<span style={{marginLeft:"auto",fontSize:10,background:"rgba(200,150,12,.2)",color:"#c8960c",padding:"1px 6px",borderRadius:8}}>{txLog.length}</span>}
              </div>
            );
          })}

          <div style={{padding:"16px 14px 6px",fontSize:10,color:"#504030",letterSpacing:2,display:"flex",justifyContent:"space-between",alignItems:"center",paddingRight:8}}>
            <span>LOCK BOXES</span>
            <button className="btn" onClick={()=>setShowNewBox(true)} style={{background:"transparent",border:"1px solid #3a3020",color:"#806040",padding:"2px 7px",fontSize:11,borderRadius:4,cursor:"pointer"}}>+</button>
          </div>
          {boxes.map(box=>{
              ...
              <div key={box} style={{borderLeft:activeBox===box?"3px solid #c8960c":"3px solid transparent"}}>
                <div className="ni" onClick={()=>setActiveBox(box)} style={{background:activeBox===box?"rgba(200,150,12,.05)":"transparent",color:activeBox===box?"#e0c080":"#706050",justifyContent:"space-between",paddingRight:8}}>
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:6}}>
                    ...
                    <button className="btn" onClick={e=>{e.stopPropagation();setEditBoxSettings({name:box,maxProps:String(bs.maxProps||DEFAULT_MAX)});}} style={{background:"transparent",border:"none",color:"#504030",fontSize:12,cursor:"pointer",padding:"0 2px"}}>⚙</button>
                  </div>
                </div>
                <div style={{padding:"0 18px 6px"}}>
                  <div className="pb" style={{marginTop:0}}><div className="pf" style={{width:`${bs.pct}%`,background:pc}}/></div>
                  <div style={{fontSize:10,color:"#504030",marginTop:2}}>{bs.props}/{bs.maxProps} props · {bs.keys} keys</div>
                </div>
              </div>
            );
          })}

          <div style={{padding:"14px 14px 8px",fontSize:10,color:"#504030",letterSpacing:2}}>KEY TYPES</div>
          {KEY_TYPES.map(k=>{
            const total=Object.values(data).flat().reduce((s,r)=>s+(r[k]||0),0);
            return (
              <div key={k} style={{padding:"4px 18px",display:"flex",justifyContent:"space-between",fontSize:12,color:"#706050"}}>
                <span>{KEY_ICONS[k]} {k.replace(" Key","")}</span>
                <span style={{color:"#c8960c"}}>{total}</span>
              </div>
            );
          })}
        </div>
        ...
        <div style={{flex:1,padding:"22px 26px",overflow:"auto"}}>
          ...
          {activeTab==="inventory"&&(
            <div>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:18}}>
                ...
                <input placeholder="Search address or lock no..." value={invSearch} onChange={e=>setInvSearch(e.target.value)} style={{width:280}}/>
              </div>
              <div style={{background:"#0a0a08",border:"1px solid #1e1e10",borderRadius:6,padding:"10px 16px",marginBottom:16,display:"flex",alignItems:"center",gap:16}}>
                <div style={{fontSize:12,color:"#806040",whiteSpace:"nowrap"}}>Capacity</div>
                <div style={{flex:1}}><div className="pb" style={{height:6}}><div className="pf" style={{width:`${activeBoxStat.pct}%`,background:pctColor(activeBoxStat.pct)}}/></div></div>
                <div style={{fontSize:12,color:pctColor(activeBoxStat.pct),whiteSpace:"nowrap"}}>{activeBoxStat.props}/{activeBoxStat.maxProps}{activeBoxStat.pct>=100?" — FULL 🔒":""}</div>
                <button className="btn btn-ghost" onClick={()=>setEditBoxSettings({name:activeBox,maxProps:String(activeBoxStat.maxProps||DEFAULT_MAX)})}>⚙ Settings</button>
              </div>
              <div style={{background:"#0c0c0a",border:"1px solid #1e1e10",borderRadius:8,overflow:"hidden"}}>
                <div style={{overflowX:"auto"}}>
                  <table>
                    <thead><tr>
                      <th>#</th><th>Property Address</th>
                      {KEY_TYPES.map(k=><th key={k}>{KEY_ICONS[k]} {k}</th>)}
                    ...
                    <tbody>
                      {filteredRows.map(row=>{
                        ...
                        return (
                          <tr key={row._id}>
                            <td style={{color:"#605040",fontSize:12}}>{row["Lock Box No."]}</td>
                            <td style={{color:"#e0d0b0",maxWidth:260}}>{row["Property Address"]}</td>
                            {KEY_TYPES.map(k=>(
                              <td key={k} style={{textAlign:"center"}}>
                                {row[k]>0?<span className={`badge ${row[k]>=3?"badge-h":"badge-a"}`}>{row[k]}</span>:<span className="badge badge-z">—</span>}
                              </td>
                            ))}
          ...
          {(activeTab==="enter"||activeTab==="withdraw")&&(
            <div style={{maxWidth:540}}>
              ...
              <div style={{display:"flex",border:"1px solid #3a3020",borderRadius:6,overflow:"hidden",marginBottom:22}}>
              ...
              <div className="fg">
                <div>
                  <div className="lbl">Property Address — searches ALL boxes</div>
                  <AddressInput value={txAddress}
                    onChange={(v,box)=>{setTxAddress(v);if(box)setTxBox(box);}}
                    onSelect={(addr,box)=>{setTxAddress(addr);setTxBox(box);}}
                    ...
                    allEntries={entries} boxes={boxes} boxStats={boxStats} activeBox={activeBox}
                    placeholder="Type address — searches all boxes..."/>
                </div>
                {txAddress&&(
                  <div style={{background:"#0a0a08",border:"1px solid #2a2010",borderRadius:6,padding:"12px 16px"}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                      <div className="lbl" style={{margin:0}}>Box</div>
                      <select value={txBox} onChange={e=>setTxBox(e.target.value)} style={{padding:"4px 8px",fontSize:12}}>
                        {boxes.map(b=><option key={b}>{b}</option>)}
                      </select>
                    </div>
                    {currentTxRow?(
                      <>
                        <div className="lbl" style={{marginBottom:6}}>Current stock — Lock Box #{currentTxRow["Lock Box No."]}</div>
                        <div style={{display:"flex",flexWrap:"wrap",gap:10}}>
                          {KEY_TYPES.map(k=>(
                            <div key={k} style={{fontSize:12,color:(currentTxRow[k]||0)>0?"#c8960c":"#404040"}}>
                              {KEY_ICONS[k]} {k}: <strong>{currentTxRow[k]||0}</strong>
                            </div>
                          ))}
                        </div>
                        {txMode==="deposit"&&(
                          <div style={{marginTop:8,fontSize:11,color:pctColor(boxStats[txBox]?.pct||0)}}>
                            Box: {boxStats[txBox]?.props}/{boxStats[txBox]?.maxProps} properties
                            <span style={{marginLeft:8,fontWeight:"bold"}}>{(boxStats[txBox]?.pct||0)>=100?"🔒 FULL — choose another box":`(${boxStats[txBox]?.pct||0}%)`}</span>
                          </div>
                        )}
                      </>
                    ):(
                      <div style={{fontSize:12,color:"#604030"}}>Address not found in {txBox} — switch box above</div>
                    )}
                  </div>
                )}
                <div><div className="lbl">Key Type</div>
                  <select value={txKeyType} onChange={e=>setTxKeyType(e.target.value)} style={{width:"100%"}}>
                    {KEY_TYPES.map(k=><option key={k}>{k}</option>)}
                  </select>
                </div>
                <div><div className="lbl">Quantity</div>
                  <input
                    ...
                    onBlur={e=>{const v=parseInt(e.target.value);if(!v||v<1)setTxQty("1");}}
                    inputMode="numeric"
                    style={{width:"100%"}}
                  />
                </div>
                <button className="btn btn-amber" onClick={handleTransaction} style={{fontSize:13,padding:"12px"}}>
                  {txMode==="withdraw"?"⬆ Withdraw Keys":"⬇ Deposit Keys"}
                </button>
              </div>
            </div>
          )}
            ...
            <div style={{maxWidth:520}}>
              ...
              <div className="fg">
                <div>
                  ...
                  <select value={addBox} onChange={e=>{setAddBox(e.target.value);setAddExisting(false);}} style={{width:"100%"}}>
                    {boxes.map(b=><option key={b}>{b}</option>)}
                  </select>
                  {boxStats[addBox]&&(
                    <div style={{fontSize:11,marginTop:4,color:pctColor(boxStats[addBox].pct),
                      background:boxStats[addBox].pct>=100?"rgba(200,80,80,.08)":"transparent",
                      padding:boxStats[addBox].pct>=100?"6px 10px":"0",borderRadius:4,
                      border:boxStats[addBox].pct>=100?"1px solid rgba(200,80,80,.2)":"none"}}>
                      {boxStats[addBox].pct>=100
                        ?<>🔒 <strong>{addBox} is FULL</strong> ({boxStats[addBox].props}/{boxStats[addBox].maxProps}) — choose another box</>
                ...
                <div>
                  ...
                  <AddressInput value={addAddress}
                    ...
                    onSelect={(addr,box)=>{
                      setAddAddress(addr);setAddBox(box);
                      setAddKeys(Object.fromEntries(KEY_TYPES.map(k=>[k,"0"])));
                      setAddExisting(true);
                    }}
                    onAddNew={(q,box)=>{setAddAddress(q);setAddBox(box);setAddExisting(false);setAddKeys(Object.fromEntries(KEY_TYPES.map(k=>[k,"0"])));}}
                    allEntries={entries} boxes={boxes} boxStats={boxStats}
                    placeholder="Enter property address..."/>
                  <div style={{fontSize:11,color:"#604030",marginTop:4}}>
                    If address exists, keys will be <strong style={{color:"#c8960c"}}>added to existing totals</strong>
                  </div>
                </div>

                {/* Show existing stock when updating */}
                {addExisting&&addAddress&&(()=>{
                  ...
                  return (
                    <div style={{background:"#0a0a08",border:"1px solid rgba(200,150,12,.3)",borderRadius:6,padding:"12px 16px"}}>
                      <div className="lbl" style={{color:"#c8960c",marginBottom:6}}>⚡ UPDATING EXISTING — Lock Box #{existRow["Lock Box No."]}</div>
                      <div style={{fontSize:10,color:"#806040",marginBottom:8}}>Current stock (amounts below will be ADDED):</div>
                      <div style={{display:"flex",flexWrap:"wrap",gap:10}}>
                        {KEY_TYPES.map(k=>(
                          <div key={k} style={{fontSize:12,color:(existRow[k]||0)>0?"#c8960c":"#404040"}}>
                            {KEY_ICONS[k]} {k}: <strong>{existRow[k]||0}</strong>
                          </div>
                        ))}
                      </div>
                    </div>
                ...
                <div style={{background:"#0a0a08",border:"1px solid #2a2010",borderRadius:6,padding:"14px 16px"}}>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
                    {KEY_TYPES.map(k=>(
                      <div key={k}>
                        <div style={{fontSize:12,color:"#806040",marginBottom:4}}>{KEY_ICONS[k]} {k}</div>
                        <input type="number" min="0" value={addKeys[k]}
                          onChange={e=>setAddKeys(prev=>({...prev,[k]:e.target.value}))}
                          onBlur={e=>{const v=parseInt(e.target.value);if(isNaN(v)||v<0)setAddKeys(prev=>({...prev,[k]:"0"}));}}
                          inputMode="numeric" style={{width:"100%"}}/>
                      </div>
                    ))}
                  </div>
                </div>
                <button className="btn btn-amber" onClick={handleAddNew}
          ...
          {activeTab==="history"&&(
            <div>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:18,flexWrap:"wrap",gap:10}}>
                <div className="sec" style={{margin:0}}>Transaction History — {filteredLog.length} records</div>
                <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                  <input placeholder="Search address, box, key type..." value={logSearch} onChange={e=>setLogSearch(e.target.value)} style={{width:260}}/>
                  <select value={logFilter} onChange={e=>setLogFilter(e.target.value)} style={{padding:"8px 12px"}}>
                  ...
                  {txLog.length>0&&(
                    <button className="btn btn-outline" onClick={()=>{
                      const wb=XLSX.utils.book_new();
                      const ws=XLSX.utils.json_to_sheet(txLog.map(l=>({Date:l.ts,Type:l.type.toUpperCase(),Address:l.address,"Lock Box":l.box,"Key Type":l.keyType,Qty:l.qty})));
                      ws["!cols"]=[{wch:18},{wch:10},{wch:42},{wch:10},{wch:16},{wch:6}];
                      XLSX.utils.book_append_sheet(wb,ws,"History");
                      XLSX.writeFile(wb,"RBR-Key_History.xlsx");showToast("History exported!");
                    }}>⬇ Export History</button>
                  )}
                </div>
              </div>
              {filteredLog.length===0?(
                <div style={{background:"#0c0c0a",border:"1px solid #1e1e10",borderRadius:8,padding:"60px 20px",textAlign:"center"}}>
                  <div style={{fontSize:32,marginBottom:12}}>📜</div>
                  <div style={{fontSize:14,color:"#604030"}}>No transactions yet</div>
                  <div style={{fontSize:11,color:"#3a2010",marginTop:6}}>Deposits, withdrawals and new entries will appear here</div>
                </div>
              ):(
                <>
                  <div style={{display:"flex",gap:12,marginBottom:18,flexWrap:"wrap"}}>
                    {[{label:"Total",val:txLog.length,c:"#c8960c"},{label:"Deposits",val:txLog.filter(l=>l.type==="deposit").length,c:"#50c880"},{label:"Withdrawals",val:txLog.filter(l=>l.type==="withdraw").length,c:"#e07070"},{label:"New Added",val:txLog.filter(l=>l.type==="add").length,c:"#7090d0"}].map(s=>(
                      <div key={s.label} className="sc" style={{flex:1,minWidth:100}}>
                        <div style={{fontSize:20,fontWeight:"bold",color:s.c}}>{s.val}</div>
                        <div style={{fontSize:10,color:"#806040",letterSpacing:1,marginTop:2}}>{s.label.toUpperCase()}</div>
                      </div>
                    ))}
                  </div>
                  <div style={{background:"#0c0c0a",border:"1px solid #1e1e10",borderRadius:8,overflow:"hidden"}}>
                        ...
                        <tbody>
                          {filteredLog.map(log=>(
                            <tr key={log.id}>
                              ...
                              <td>
                                {log.type==="deposit"&&<span className="badge badge-dep">⬇ Deposit</span>}
                                {log.type==="withdraw"&&<span className="badge badge-wit">⬆ Withdraw</span>}
                                {log.type==="add"&&<span className="badge badge-add">➕ New</span>}
                              </td>
                              <td style={{color:"#e0d0b0",maxWidth:260}}>{log.address}</td>
      ...
      {/* ── New Box Modal ── */}
      {showNewBox&&(
        <div className="mo" onClick={()=>setShowNewBox(false)}>
          <div className="md" style={{width:400}} onClick={e=>e.stopPropagation()}>
            <div style={{fontSize:13,color:"#c8960c",fontWeight:"bold",marginBottom:18,letterSpacing:1}}>📦 CREATE NEW LOCK BOX</div>
            <div className="fg">
              <div><div className="lbl">Box Name</div>
                <input value={newBoxName} onChange={e=>setNewBoxName(e.target.value.toUpperCase())} placeholder="e.g. BOX 3" style={{width:"100%"}} autoFocus/>
              </div>
              <div><div className="lbl">Max Properties Allowed</div>
                <input type="number" min="1" value={newBoxMax}
                  onChange={e=>setNewBoxMax(e.target.value)}
                  onBlur={e=>{const v=parseInt(e.target.value);if(!v||v<1)setNewBoxMax("1");}}
                  inputMode="numeric" style={{width:"100%"}}/>
                <div style={{fontSize:11,color:"#604030",marginTop:4}}>Box will be blocked when this property count is reached</div>
              </div>
              <div style={{display:"flex",gap:10}}>
                <button className="btn btn-amber" onClick={handleAddBox} style={{flex:1,padding:"11px"}}>Create Box</button>
      ...
      {/* ── Box Settings Modal ── */}
      {editBoxSettings&&(
        <div className="mo" onClick={()=>setEditBoxSettings(null)}>
          <div className="md" style={{width:400}} onClick={e=>e.stopPropagation()}>
            <div style={{fontSize:13,color:"#c8960c",fontWeight:"bold",marginBottom:6,letterSpacing:1}}>⚙ {editBoxSettings.name} SETTINGS</div>
            <div style={{fontSize:12,color:"#806040",marginBottom:18}}>Current: {boxStats[editBoxSettings.name]?.props||0} properties in this box</div>
            <div className="fg">
              <div><div className="lbl">Max Properties Allowed</div>
                <input type="number" min="1" value={editBoxSettings.maxProps}
                  onChange={e=>setEditBoxSettings(s=>s?{...s,maxProps:e.target.value}:null)}
                  onBlur={e=>{const v=parseInt(e.target.value);if(!v||v<1)setEditBoxSettings(s=>s?{...s,maxProps:"1"}:null);}}
                  inputMode="numeric"
                  style={{width:"100%"}}/>
                <div style={{fontSize:11,color:"#604030",marginTop:4}}>Must be ≥ current count ({boxStats[editBoxSettings.name]?.props||0})</div>
              </div>
              <div style={{display:"flex",gap:10}}>
      ...
      {/* ── Edit Row Modal ── */}
      {editRow&&(
        <div className="mo" onClick={()=>setEditRow(null)}>
          <div className="md" onClick={e=>e.stopPropagation()}>
            <div style={{fontSize:13,color:"#c8960c",fontWeight:"bold",marginBottom:16,letterSpacing:1}}>✏ EDIT — {editRow["Property Address"]}</div>
            <div className="fg">
              <div><div className="lbl">Property Address</div>
                <input value={editRow["Property Address"]} onChange={e=>setEditRow(r=>r?{...r,"Property Address":e.target.value}:r)} style={{width:"100%"}}/>
              </div>
              <div><div className="lbl">Lock Box No.</div>
                <input type="number" value={editRow["Lock Box No."]} inputMode="numeric"
                onChange={e=>{const raw=e.target.value;setEditRow(r=>r?{...r,"Lock Box No.":raw===""?0:parseInt(raw)||r["Lock Box No."]}:r);}}
                onBlur={e=>{if(!e.target.value||parseInt(e.target.value)<0)setEditRow(r=>r?{...r,"Lock Box No.":0}:r);}}
                style={{width:"100%"}}/>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
                {KEY_TYPES.map(k=>(
                  <div key={k}><div className="lbl">{KEY_ICONS[k]} {k}</div>
                    <input type="number" min="0" value={editRow[k]} inputMode="numeric"
                      onChange={e=>{const raw=e.target.value;setEditRow(r=>r?{...r,[k]:raw===""?0:parseInt(raw)||0}:r);}}
                      onBlur={e=>{if(e.target.value===""||parseInt(e.target.value)<0)setEditRow(r=>r?{...r,[k]:0}:r);}}
                      style={{width:"100%"}}/>
                  </div>
                ))}
              </div>
      ...
      {/* ── Toast ── */}
      {toast&&(
        <div style={{position:"fixed",bottom:22,right:22,zIndex:2000,
          background:toast.type==="error"?"#3a0a0a":"#0a1a0a",
          border:`1px solid ${toast.type==="error"?"#8a2020":"#207020"}`,
          color:toast.type==="error"?"#ff8080":"#80d080",
          padding:"11px 20px",borderRadius:8,fontSize:13,
          fontFamily:'"Courier New",monospace',boxShadow:"0 4px 20px rgba(0,0,0,.5)",
          animation:"slideIn .2s ease",maxWidth:400}}>
          {toast.msg}
        </div>
      )}
    </div>
